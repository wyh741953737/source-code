(function () {
  const app = angular.module('disputePolicyApp', ['service', 'home-service', 'utils', 'CommonHeaderCom', 'CommonFooterCom', 'commonRelatedLinks', 'custom-filter', 'cjCompnentModule']);
  app.controller('disputePolicyCtrl', ['$scope', 'dsp', '$sce', '$http', function ($scope, dsp, $sce, $http) {
    $scope.termsList = [];
    $scope.interList = [];
    $scope.showItem = null;
    $http.get('/static/json/disputePolicy.json').then(function(data) {
      $scope.termsList = data.data.RefundCase;
      $scope.interList = data.data.ImportantInterpretation;
      layer.closeAll('loading');
    })
    $scope.handleShowItem = (item) => {
