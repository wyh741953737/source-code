(function() {
  //  var module = angular.module('app-passWordEmail', ['service']);//创建一个模块
    var module = angular.module('app-passwordemail', ['service']);
    module.controller('passWordEmailCtrl', ['$scope',  'dsp', function($scope, dsp) {
        ...
        const passwordStatusMsg = {
            ...
            5:'Please passwords do not match.'
        }
        $scope.passwordInput = ()=>{
        ...
        $scope.setPassword = ()=>{
            ...
            dsp.postFun(`app/account/updatepwd`,param, ({data})=> {
                ...
                if (data.statusCode == '200') {
                    ...
                    layer.msg('Similar password! Please reset a password');
                }else if(data.statusCode == '606'){
                    layer.msg('Please verify your email address first');
                }else{
                    layer.msg('Save failed. Please try again')
                }
            }, ()=> {
                layer.closeAll('loading');
            });
