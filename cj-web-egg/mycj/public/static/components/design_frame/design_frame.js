(function(angular) {
    ...
    angular.module('cjCompnentModule')
        .component('designFrame', {
            templateUrl: 'static/components/design_frame/design_frame.html',
            controller: ['$scope', 'dsp', function ($scope, dsp) {
                this.$onInit = function () {
                    if (typeof wangEditor === 'undefined') {
                        window.addEventListener('wangEditor.min.js', function() {
    ...
    function designFrameCtrl ($scope, dsp) {
        ...
        $scope.submit = function () {
            ...
            var finalVitems = getFinalVitems();

            for (var i = 0; i < finalVitems.length; i++) {
                if (!oriSkuMap[finalVitems[i].vItem]) {
                    // 临时修复找不到原变体问题
                    ...
                    layer.msg(
                        ...
                        The variant 
                        "${finalVitems[i].vItem}"
                        doesn’t exist. Please don’t select the
                        "${finalVitems[i].vItem}"
                        option.
                        `
                    )
        ...
        function getCheckItems() {
            ...
            if ($scope.varientArr.length == 1) {
                ...
                for (var i = 0; i < $scope.varientArr[0].key.length; i++) {
                    if ($scope.varientArr[0].key[i].check) {
                        arr1.push($scope.varientArr[0].key[i].val);
                    }
                }
                checkItemArr = arr1;
            ...
            if ($scope.varientArr.length == 3) {
                ...
                var arr3 = [];
                for (var i = 0; i < $scope.varientArr[2].key.length; i++) {
                    if ($scope.varientArr[2].key[i].check) {
                        arr3.push($scope.varientArr[2].key[i].val);
                    }
                }
                for (var m = 0; m < arr1.length; m++) {
                    for (var n = 0; n < arr2.length; n++) {
                        for (var o = 0; o < arr3.length; o++) {
                            checkItemArr.push(arr1[m] + '-' + arr2[n] + '-' + arr3[o]);
                        }
                    }
                }
                console.log(checkItemArr);
        ...
        // var checkItemArr = getCheckItems();
        // var finalVitems = [];
        function getFinalVitems () {
            var finalVitems = [];
            var checkItemArr = getCheckItems && getCheckItems();
            if ($scope.podColor) {
                var checkColorArr = getCheckColors && getCheckColors();
