(function(angular) {
    ...
    angular.module('cjCompnentModule')
        .component('designFrame', {
            templateUrl: 'static/components/design_frame/design_frame2.html',
    ...
    function designFrameCtrl ($scope, dsp) {
        ...
        function dongQilai (flag) {
            // if you have multiple .draggable elements
            // get all draggie elements
            var draggableElems;
            if (flag==1) {
                draggableElems = document.querySelectorAll('.draggable-front');
            }
            if (flag==2) {
                draggableElems = document.querySelectorAll('.draggable-back');
            }
        ...
        $scope.$on('showdesignframe', function(d,data) {  
            ...
            dsp.postFun('cj/locProduct/huoQuShangPinXiangQing', JSON.stringify({
                ...
                // 删除
                // var temObj = {"color":{"range":"150","colors":[{"nameCn":"粉色","nameEn":"Pink","value":"#FFC1C1"},{"nameCn":"蓝色","nameEn":"Blue","value":"#B0E2FF"}]},"zone":{"front":{"imgurl":"https://cc-west-usa.oss-us-west-1.aliyuncs.com/2048/6840699695101.png","podtype":"pic"},"back":{"imgurl":"https://cc-west-usa.oss-us-west-1.aliyuncs.com/2048/548180687380.png","podtype":"text"}}};
        ...
        function getPosiInfo(flag1, callback) {
            ...
            function imageReceived() {
                ...
                newDom.style='position: absolute; left: '+temPosiArr[0]+'px; top: '+temPosiArr[1]+'px; width: '+temPosiArr[2]+'px; height: '+temPosiArr[3]+'px; overflow: hidden; z-index:1;'
                if (flag1) {
                    ...
                    newDom.className = 'front-container';
                    document.querySelector('#img-box-front').append(newDom);
                } else {
                    ...
                    newDom.className = 'back-container';
                    document.querySelector('#img-box-back').append(newDom);
                }
        ...
        var currentMoveDom, currentMoveDomLeft, currentMoveTop, currentMoveDomWidth, currentMoveDomHeight;
        var deleteDom = document.querySelector('.asj-resize-delete-btn');
        var deleteDomLeft, deleteDomTop;
        var resizeDom = document.querySelector('.asj-resize-resize-btn')
        var resizeDomLeft, resizeDomTop;
        var draggie = new Draggabilly(document.querySelector('.asj-resize-box'), {
            // options...
            // containment: '.front-container'
        });
        var resizeBox = document.querySelector('.asj-resize-box');
        draggie.on( 'dragStart', function( event, pointer ) {
            currentMoveDomLeft = currentMoveDom.offsetLeft;
            currentMoveTop = currentMoveDom.offsetTop;
            deleteDomLeft = deleteDom.offsetLeft;
            deleteDomTop = deleteDom.offsetTop;
            resizeDomLeft = resizeDom.offsetLeft;
            resizeDomTop = resizeDom.offsetTop;
        ...
        draggie.on( 'dragMove', function( event, pointer, moveVector ) {
            ...
            console.log(currentMoveDom)
            currentMoveDom.style.left = currentMoveDomLeft + moveVector.x + 'px';
            currentMoveDom.style.top = currentMoveTop + moveVector.y + 'px';
            deleteDom.style.left = deleteDomLeft + moveVector.x + 'px';
            deleteDom.style.top = deleteDomTop + moveVector.y + 'px';
            resizeDom.style.left = resizeDomLeft + moveVector.x + 'px';
            resizeDom.style.top = resizeDomTop + moveVector.y + 'px';
        });
        draggie.on( 'dragEnd', function( event, pointer ) {
            currentMoveDomLeft = null;
            currentMoveTop = null;
            deleteDomLeft = null;
            deleteDomTop = null;
            resizeDomLeft = null;
        ...
        resizeDom.onmousedown = function (event) {
            var event = event || window.event;
            //获取鼠标在页面中的位置
            var pageX = event.pageX || event.clientX + document.documentElement.scrollLeft;
            var pageY = event.pageY || event.clientY + document.documentElement.scrollTop;
            currentMoveDomWidth = currentMoveDom.offsetWidth;
            currentMoveDomHeight = currentMoveDom.offsetHeight;
            deleteDomLeft = deleteDom.offsetLeft;
            deleteDomTop = deleteDom.offsetTop;
            resizeDomLeft = resizeDom.offsetLeft;
            resizeDomTop = resizeDom.offsetTop;
            ...
            document.onmousemove = function (event) {
                var event = event || window.event;
                var pageXNew = event.pageX || event.clientX + document.documentElement.scrollLeft;
                var pageYNew = event.pageY || event.clientY + document.documentElement.scrollTop;
                var moveX = pageXNew - pageX;
                var moveY = pageYNew - pageY;
                currentMoveDom.style.width = currentMoveDomWidth + moveX + 'px';
                currentMoveDom.style.height = Math.floor((currentMoveDomWidth + moveX) * currentMoveDomHeight / currentMoveDomWidth) + 'px';
                resizeBox.style.width = currentMoveDomWidth + moveX + 'px';
                resizeBox.style.height = Math.floor((currentMoveDomWidth + moveX) * currentMoveDomHeight / currentMoveDomWidth) + 'px';
                deleteDom.style.left = deleteDomLeft + moveX + 'px';
                resizeDom.style.left = resizeDomLeft + moveX + 'px';
                resizeDom.style.top = resizeDomTop + Math.floor((currentMoveDomWidth + moveX) * currentMoveDomHeight / currentMoveDomWidth) - currentMoveDomHeight + 'px';
                console.log(currentMoveDomWidth,moveX)
            };           
        }
        //鼠标弹起 盒子就不应该跟着了
        document.onmouseup = function () {
            document.onmousemove = null;
            currentMoveDomWidth = null;
        };
            ...
            reader.onload = function (e) {
                ...
                image.onload = function () {
                    ...
                    if (width >= 1000 && height >= 1000) {
                        ...
                        if (width > $scope.posiArrFront[2]) {
                            sizeWidth = $scope.posiArrFront[2];
                            sizeHeight = Math.floor(height * sizeWidth / width);
                        } else {
                           sizeWidth = width; 
                           sizeHeight = height;
                        }
                        if (sizeHeight > $scope.posiArrFront[3]) {
                            sizeWidth = Math.floor(sizeWidth * $scope.posiArrFront[3] / sizeHeight);
                            sizeHeight = $scope.posiArrFront[3];
                        }
                        $scope.podArray.front.image.push({
                            id: 'frontimage'+fIndex,
                            base64: result,
                            size: {
                                width: width,
                                height: height
                        ...
                        // newDivDom.className = 'image-move-wrap';
                        // newDivDom.style='position: absolute; left: '+$scope.posiArrFront[0]+'px; top: '+$scope.posiArrFront[1]+'px; width: '+sizeWidth+'px; height: '+sizeHeight+'px; z-index: '+fIndex+';';
                        var newDom = document.createElement('img');
                        newDom.className = 'asj-move-item';
                        newDom.style='position: absolute; left: '+Math.floor(($scope.posiArrFront[2]-sizeWidth)/2)+'px; top: '+Math.floor(($scope.posiArrFront[3]-sizeHeight)/2)+'px; width: '+sizeWidth+'px; height: '+sizeHeight+'px; z-index: '+fIndex+';';
                        newDom.src = result;
                        document.querySelector('.front-container').append(newDom);
                        // var draggie = new Draggabilly(newDom, {
                        //     // options...
                        //     // containment: '.front-container'
                        // });
                        currentMoveDom = newDom;
                        document.querySelector('.asj-resize-box').style.width = sizeWidth + 'px';
                        document.querySelector('.asj-resize-box').style.height = sizeHeight + 'px';
                        document.querySelector('.asj-resize-box').style.top = document.querySelector('.front-container').offsetTop + Math.floor(($scope.posiArrFront[3]-sizeHeight)/2) + 'px';
                        document.querySelector('.asj-resize-box').style.left = document.querySelector('.front-container').offsetLeft + Math.floor(($scope.posiArrFront[2]-sizeWidth)/2) + 'px';
                        document.querySelector('.asj-resize-box').style.display = 'block';
                        document.querySelector('.asj-resize-delete-btn').style.top = document.querySelector('.front-container').offsetTop + Math.floor(($scope.posiArrFront[3]-sizeHeight)/2) - 19 + 'px';
                        document.querySelector('.asj-resize-delete-btn').style.left = document.querySelector('.front-container').offsetLeft + Math.floor(($scope.posiArrFront[2]-sizeWidth)/2) + sizeWidth - 4 + 'px';
                        document.querySelector('.asj-resize-delete-btn').style.display = 'block';
                        document.querySelector('.asj-resize-resize-btn').style.top = document.querySelector('.front-container').offsetTop + Math.floor(($scope.posiArrFront[3]-sizeHeight)/2) + sizeHeight + 'px';
                        document.querySelector('.asj-resize-resize-btn').style.left = document.querySelector('.front-container').offsetLeft + Math.floor(($scope.posiArrFront[2]-sizeWidth)/2) + sizeWidth + 'px';
                        document.querySelector('.asj-resize-resize-btn').style.display = 'block';
                ...
                if (data.data.statusCode == 200) {
                    if (that.parentctrl == 'productdetail') {
                        layer.open({
                            ...
                            shadeClose: true,
                            skin: '',
                            content: '<p class="pdlist-p1">The product you designed has been added. You can find it in My CJ > Print on Demand > My Design.</p>'
                            + '<p class="pdlist-p2"><a href="' + 'myCJ.html#/pod/design-myself' + '" target="_blank">View</a></p>',
