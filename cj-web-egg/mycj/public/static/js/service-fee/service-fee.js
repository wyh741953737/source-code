(function () {
  const app = angular.module('servicezFeeApp', ['service', 'home-service', 'utils', 'CommonHeaderCom', 'CommonFooterCom', 'commonRelatedLinks', 'custom-filter', 'cjCompnentModule']);
  app.controller('servicezFeeCtrl', ['$scope', 'dsp', '$sce', '$http', function ($scope, dsp, $sce, $http) {
    $scope.showFee = false;
    $scope.allServiceFeeObject = {};
    $scope.serviceFee = {};
    $http.get('/static/json/serviceFee.json').then(function(data) {
      $scope.allServiceFeeObject = data.data;
      $scope.serviceFee = data.data.US;
      // layer.closeAll('loading');
      $scope.showFee = true;
    })
    $scope.isClick = "US";
    $scope.handleChange = (val) => {
      layer.load(2);
      $scope.isClick = val;
      switch(val) {
        case 'US':
          $scope.serviceFee = $scope.allServiceFeeObject.US;
          break;
        case 'Germany':
          $scope.serviceFee = $scope.allServiceFeeObject.Germany;
          break;
        case 'Thailand':
          $scope.serviceFee = $scope.allServiceFeeObject.Thailand;
          break;
        case 'Indonesia':
          $scope.serviceFee = $scope.allServiceFeeObject.Indonesia;
          break;
        case 'China':
          $scope.serviceFee = $scope.allServiceFeeObject.China;
          break;
        default:
          $scope.isClick = "US";
          $scope.serviceFee = $scope.allServiceFeeObject.US;
