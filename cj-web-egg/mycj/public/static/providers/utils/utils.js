; (function () {
  ...
  angular.module('utils', []).service('utils', [function () {
    ...
    this.checkLogin = () => {
      ...
      const loginName = this.storage.get('loginName');
      // const firstName = store.get('firstName');
      const loginTime = this.storage.get('loginTime');
      const removeLogin = () => {
        this.storage.remove(['loginName', 'firstName', 'loginTime', 'token', 'cjLoginName', 'cjLoginToken']);
      };
      ...
      this.setCookie('cjLoginName', this.base64.decode(loginName), 3, window.__root__domain);
      this.setCookie('cjLoginToken', this.base64.decode(token), 3, window.__root__domain);

      this.storage.set('loginTime', nowTime);
      return true;
    }

    this.setCookie = (c_name, value, expiredays, domain, protocol) => {
      var exdate;
      if (expiredays) {
        exdate = new Date()
