export function dropClosedFactory(angular) {
  const app = angular.module('drop-closed.module', ['service']);

  app.controller('drop-closed.ctrl', ['$scope', 'dsp',
    function ($scope, dsp) {
        ...
        dsp.postFun('app/order/queryShipmentsOrder', JSON.stringify(dapData), function (data) {
          ...
          if (notMuOrdNumFlag) {
            if ($scope.erpordTnum > 0) {
              ...
              ziid = bs.encode(ziid);
              location.href = `#/drop-close-zi/${muordid}/${ziid}`;
            }
          }
      ...
      $scope.ziordFun = function (item) {
        ...
        location.href = `#/drop-close-zi/${muordid}`;
      ...
      $scope.gotozifun = function (item, stu) {
        ...
        location.href = `#/drop-close-zi/${muordid}//${stu}`;
      }
      //导出订单
