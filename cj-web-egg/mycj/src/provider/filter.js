...
export function filterFactory(module) {

  /** 阿里雲圖片大小處理 */
  module.filter('IMG_SIZE', function () {
  ...
  /** 圖片切割 */
  module.filter('imgSeparator', function () {
    return function (url) {
      if (typeof url !== 'string') return url;
      try {
        return url.split(',')[0]
  ...
  /** 首字母大写 */
  module.filter('fWordUpcase', function () {
  ...
  /** 国旗 */
  module.filter('nationalFlag', function () {
    return function (str) {
   ...
   */
  module.filter('exchangeRate', function () {
  ...
  /** 價格加工 */
  module.filter('priceProcessor', function () {
  ...
  module.filter('strorageFil', function () { //可以注入依赖
    return function (val) {
      let text = '';
      if (val == 0) {
        text = 'Yiwu Warehosue';
