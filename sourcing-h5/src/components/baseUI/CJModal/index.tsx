...
export default (props: IProps) => {
  ...
  const buttons = (
    <>
      <CJText
        onClick={handleOk}
        h5Class={`${styles.modal_button} ${activeButtonIndex === 0 ? styles.active_color : ''}`}
      >
        {okText}
      </CJText>
      <CJText
        onClick={handleCancel}
        h5Class={`${styles.modal_button} ${activeButtonIndex === 1 ? styles.active_color : ''}`}
      >
        {cancelText}
      </CJText>
    </>
    ...
    <>
      ...
      <CJView h5Class={styles.modal_mask}>
        <CJView h5Class={`${styles.modal_warp} ${contentStyles}`}>
          ...
          <CJView h5Class={`${styles.modal_content} modal_content_wrap`}>{content}</CJView>
