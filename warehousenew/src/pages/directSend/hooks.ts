...
export const indexHooks = (warehouseId: string) => {
  ...
  const containerEnter = (e: any) => {
    ...
    const values = searchForm.form.getFieldsValue();
    ...
    if (
      ...
      const checker = (
        ...
        let target = statistics.list.find((s, i) => {
          const flag =
            String(s.sku) === String(id) || String(s.variantSku) === String(id);
          if (flag) index = i;
          return flag;
        });
        return [index, target];
      ...
      index !== undefined && statistics.list.splice(index, 1);
  ...
  const rowClassName = (record: GetList.ListItem) => {
    if (record.active) return style.active;
  };
  // url代表是否需要做下一步封箱详情弹框处理，否则不需要
  ...
  const doPackage = async (isLast: boolean = false) => {
    ...
    // consumableModal.show({ id: resp.data, isLast });   //暂时隐藏耗材
  ...
  const submitAbnormal = () => {
    ...
    Modal.confirm({
      ...
      cancelText: '关闭',
      onOk: async () => {
        const hide = message.loading('提交异常中...');
        try {
          await api.submitAbnormal(
