...
export default connect(({ exceptionPurchaseDealing }: any) => ({
  ...
  dataSource: exceptionPurchaseDealing.dataSource,
  current: exceptionPurchaseDealing.current,
  pageSize: exceptionPurchaseDealing.pageSize,
  total: exceptionPurchaseDealing.total,
  loading: exceptionPurchaseDealing.loading,
}))(({ dataSource, loading, current, pageSize, total, dispatch }) => {
  const { onChange, dealModal, dealConfirmModal } = indexHooks(dispatch);
    ...
    const columns1 = defaultColumns([
      ...
      },
      {
        ...
        render: (text: number) => {
          return (
            <Space>
              <AuthJudge code={AUTH.YCCL008001}>
                <Button
                  size="small"
                  type="primary"
              ...
              <AuthJudge code={AUTH.YCCL008002}>
                <Button
                  size="small"
                  type="primary"
                  onClick={() => dealConfirmModal.show({ id: text })}
                >
                  确认处理
                </Button>
              </AuthJudge>
