 ...
 * 异常记录
 */
export default connect(({ exceptionWarehouseProcessed }: any) => ({
  searchData: exceptionWarehouseProcessed.searchData,
  dataSource: exceptionWarehouseProcessed.dataSource,
  current: exceptionWarehouseProcessed.current,
  pageSize: exceptionWarehouseProcessed.pageSize,
  total: exceptionWarehouseProcessed.total,
  loading: exceptionWarehouseProcessed.loading,
}))(({ dataSource, loading, current, pageSize, total, dispatch }) => {
  const { onChange, dealResultModal } = indexHooks(dispatch);
        ...
        render: (text: number) => {
          return (
            <Button
              type="primary"
              size="small"
              onClick={() => dealResultModal.show({ id: text })}
            >
              查看详情
            </Button>
          );
  ...
  return (
    <SearchTable searchFormRender={<SearchForm />}>
      ...
      />
      <DealResult modal={dealResultModal} />
    </SearchTable>
  );
});
