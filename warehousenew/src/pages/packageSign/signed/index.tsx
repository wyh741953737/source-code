...
export default connect(({ packageSigned }: any) => ({
  dataSource: packageSigned.dataSource,
  loading: packageSigned.loading,
  current: Number(packageSigned.current),
  pageSize: Number(packageSigned.pageSize),
  total: packageSigned.total,
      ...
      {record.detailsList.length > 0 &&
        record.detailsList.map((item: any) => (
          <div key={item.id} className={style['expanded-item']}>
            ...
            <span>已质检数量：{item.inspectionQuantity || '-'}</span>
            <span>正品：{item.inspectGoodsQuantity || '-'}</span>
            <span>次品：{item.inspectInferiorQuantity || '-'}</span>
            <span>多货：{item.inspectManyGoodsQuantity || '-'}</span>
            <span>少货：{item.inspectSmallQuantity || '-'}</span>
          </div>
        ))}
    </>
  );
