...
import { Descriptions, Image, Tooltip, Tag } from 'antd';
...
export default ({ data, status, showCustomerInfo }: Props) => {
  const cls = classname({
        ...
        <Descriptions column={2} size="small">
          <Descriptions.Item label="SKU">
            <Tooltip placement="topLeft" title={data.variantSku}>
              {data.variantSku}
              <br />
              {data.variantSku[0].toLocaleUpperCase() == 'P' && (
                <Tag
                  color="#f50"
                  style={{ cursor: 'pointer' }}
                  onClick={() => showCustomerInfo(data)}
                >
                  个
                </Tag>
              )}
